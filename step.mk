...
%.sage %.sage.mk : %.sage.step $(SageDynamics)/step.mk $(SageDynamics)/step.pl
	perl -f $(SageDynamics)/step.pl $*.sage '#'

%.py %.py.mk : %.py.step $(SageDynamics)/step.mk $(SageDynamics)/step.pl
	perl -f $(SageDynamics)/step.pl $*.py '#'
...
# $(SageDynamics) then we know where we are, otherwise suppose that . is 
